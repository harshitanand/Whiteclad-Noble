...
organizationSchema.virtual('features').get(function() {
  const planFeatures = {
    ...
    [SUBSCRIPTION_PLANS.PRO]: ['basic_agents', 'advanced_agents', 'api_access', 'priority_support', 'analytics'],
...
organizationSchema.methods.checkLimits = function(type) {
  ...
  const limit = this.limits[`max${type.charAt(0).toUpperCase() + type.slice(1)}`] || this.limits[type];
